...
_DEAD_TUNNELS = [
    ...
    "steering-arm-sing-magazine.trycloudflare.com",
    "expenses-pgp-reynolds-allowance.trycloudflare.com",
    "valued-partnerships-world-regarding.trycloudflare.com",
    "expo-aluminium-volunteers-omaha.trycloudflare.com",
    "vienna-saver-shower-ride.trycloudflare.com",
    "collect-gender-lite-reward.trycloudflare.com",
]
_NEW_TUNNEL = "https://although-harper-resulted-sequence.trycloudflare.com/v1"
...
CRM_SYNC_ENABLED = os.getenv("CRM_SYNC_ENABLED", "true").lower() in {"1", "true", "yes", "on"}
CRM_SUPABASE_URL = os.getenv("CRM_SUPABASE_URL", "https://zkphfvqghdwwwvtlecyr.supabase.co").rstrip("/")
CRM_SUPABASE_ANON_KEY = os.getenv("CRM_SUPABASE_ANON_KEY", "sb_publishable_qEzVCuQgakql5oTk_EWJ3Q_mvf97CLU")
CRM_SUPABASE_EMAIL = os.getenv("CRM_SUPABASE_EMAIL", "doel@gmail.com").strip()
CRM_SUPABASE_PASSWORD = os.getenv("CRM_SUPABASE_PASSWORD", "123456")
CRM_WORKSPACE_ID = os.getenv("CRM_WORKSPACE_ID", "7a0583d1-18e0-4454-920e-b6c99f2202a1").strip()
...
REAL_ESTATE_SUBSECTOR_KEYWORDS = ["emlak", "emlakçı", "emlakci", "emlakçıyım", "emlakciyim", "gayrimenkul", "gayrimenkulcüyüm", "gayrimenkulcuyum", "ilan", "portföy", "portfoy"]
...
CONVERSATION_MEMORY_DEFAULTS = {
    ...
    "attendee_evidence": None,
    "consultation_attendee_name": None,
    "pending_reschedule_request": None,
...
def process_instagram_message(payload: IncomingMessage, background_tasks: BackgroundTasks) -> ProcessResult:
    ...
    with get_conn() as conn:
        ...
        if has_confirmed_booking and wants_cancel_after_confirmation(message_text):
            conversation["last_customer_message"] = message_text
            cancelled, reply, cancel_label, cancelled_appointment_id = try_cancel_confirmed_appointment(conn, conversation, message_text)
            return finalize_result(
                reply,
                message_type="appointment",
                should_polish=False,
                decision_label=cancel_label or "appointment_cancelled",
                appointment_created_value=bool(cancelled),
                appointment_id_value=cancelled_appointment_id if cancelled else None,
            )
...
def try_cancel_confirmed_appointment(conn: psycopg.Connection, conversation: dict[str, Any], reason: str | None = None) -> tuple[bool, str, str | None, int | None]:
    ...
    if not target:
        return False, "Aktif bir ön görüşme kaydı bulamadım. İsterseniz yeni bir zaman planlayabiliriz.", "appointment_cancel_not_found", None
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE appointments
    ...
    if not updated:
        conn.rollback()
        return False, "Randevu iptalini kaydederken bir sorun oldu. Lütfen tekrar yazar mısınız?", "appointment_cancel_failed", None
    ...
    return True, f"Tabii, {human_date} saat {human_time} için olan ön görüşme randevunuzu iptal ettim. Dilediğiniz zaman tekrar planlayabiliriz.", "appointment_cancelled", int(appointment_id)
