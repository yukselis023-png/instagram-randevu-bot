...
def rewrite():
    try:
        ...
        router_pattern = re.compile(r"def apply_ai_first_quality_overrides\(.*?return []{0,1}decision", re.DOTALL)
        guard_pattern = re.compile(r"def guard_and_repair_final_answer\(.*?return {[\s]*\"ok\"[^\}]*}", re.DOTALL)
    ...
    # ==== 2. bot identity & 3. person question ====
    ...
    # ==== 9. service list follow-up (Detailed Services) ====
    ...
    # ==== 12. customer acquisition goal / DISAMBIGUATION ====
    ...
    return {"ok": True, "reasons": [], "repaired": _reply}"""
        ...
        code = router_pattern.sub(new_overrides, code, count=1)
        code = guard_pattern.sub(new_guard, code, count=1)
